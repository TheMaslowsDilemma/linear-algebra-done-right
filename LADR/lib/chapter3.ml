...
let excercise_3A_P1 () =
  ...
  Printf.printf "T (%s) + T (%s) = %s\n" (to_string v1) (to_string v2)
    (to_string sum_tv1_tv2);
  Printf.printf "T (%s + %s) = T (%s) = %s\n\n" (to_string v1) (to_string v2)
    (to_string sum_v1_v2) (to_string t_sum_v1_v2)
...
let excercise_3A_P8 () =
  ...
  let rho v =
    let x =
      (nth v 0 ** 2.0) *. nth v 1 /. ((nth v 0 ** 2.0) +. (nth v 1 ** 2.0))
    in
    of_list [ x ]
  in
